...
AMAZON_CAREERS_URL = "https://www.amazon.jobs"
AMAZON_SEARCH_URLS = (
    "https://www.amazon.jobs/en/search?base_query=agent&loc_query=Germany",
    "https://www.amazon.jobs/en/search?base_query=llm&loc_query=Germany",
    "https://www.amazon.jobs/en/search?base_query=rag&loc_query=Germany",
)
AMAZON_JOB_DOMAINS = ('amazon.jobs',)
...
class AmazonCollector:
    ...
    def collect(self) -> list[Job]:
        try:
            ...
            raise RuntimeError(
                "AmazonCollector requires Playwright. Install it with "
    ...
    def _looks_like_job_url(url: str, title: str) -> bool:
        ...
        if not title or len(title) < 4:
            return False
        if not any(domain in lower_url for domain in AMAZON_JOB_DOMAINS):
            return False
